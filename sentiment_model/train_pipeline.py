...
from sentiment_model.processing.features import (
    preprocess_text,  # Import the preprocess_text function
)
...
def train():
    ...
    # Read the dataset
    data = pd.read_csv(
        config["data"]["dataset_path"], encoding="ISO-8859-1", header=None
    )
    ...
    dump(
        pipeline,
        f"{config['data']['saving_path']}{config['model']['name']}_{config['model']['version']}.pkl",
    )
    logger.info(
        f"Model trained and saved as {config['model']['name']}_{config['model']['version']}.pkl"
    )
